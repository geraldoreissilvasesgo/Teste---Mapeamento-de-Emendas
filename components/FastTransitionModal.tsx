...
export const FastTransitionModal: React.FC<FastTransitionModalProps> = ({ 
  ...
  const handleUpdateMovement = (index: number, field: keyof AmendmentMovement, value: any) => {
    ...
    if (field === 'daysSpent' && mov.dateIn) {
      const dIn = new Date(mov.dateIn);
      dIn.setDate(dIn.getDate() + parseInt(value || 0));
      mov.dateOut = dIn.toISOString();
    }
  ...
  const handleAddMovement = () => {
    ...
    const nextDate = lastMov?.dateOut || new Date().toISOString();
    ...
    const newMov: AmendmentMovement = {
      ...
      dateIn: nextDate,
      dateOut: null,
      deadline: new Date(new Date(nextDate).getTime() + 5 * 86400000).toISOString(),
      daysSpent: 0,
      handledBy: 'Ajuste Rápido',
      analysisType: 'Tramitação Editada'
    };
  ...
  const handleSave = () => {
    ...
    // Verificação de Lock
    if (newStatusVal === Status.COMMITMENT_LIQUIDATION || finalStatuses.some(s => s.name === newStatusVal)) {
      if (!window.confirm("⚠️ ATENÇÃO: Ao selecionar um status final ou 'EMPENHO / LIQUIDAÇÃO', este registro será TRAVADO para futuras edições. Deseja confirmar o bloqueio definitivo deste processo?")) {
        return;
      }
    }
  ...
  return (
    <div className="fixed inset-0 z-[250] flex items-center justify-center bg-[#0d457a]/90 backdrop-blur-xl p-4">
      <div className="bg-white rounded-[48px] w-full max-w-4xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 border-t-8 border-amber-500 flex flex-col max-h-[90vh]">
        <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-xl font-black text-[#0d457a] uppercase tracking-tighter">Editor de Fluxo Rápido</h3>
              <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest mt-1">Correção de Ciclo e Bloqueio Financeiro</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-10 custom-scrollbar space-y-8">
          <div className="bg-blue-50 p-6 rounded-3xl border border-blue-100 flex items-start gap-4">
            ...
            <p className="text-[10px] text-blue-700 font-bold uppercase leading-relaxed">
              Use esta ferramenta para corrigir o histórico ou aplicar a trava de EMPENHO / LIQUIDAÇÃO. Registros em estados finais não permitem novas alterações.
            </p>
          </div>
          ...
          <div className="space-y-4">
            {editedMovements.map((mov, idx) => (
              <div key={mov.id} className="relative flex gap-6 group animate-in slide-in-from-left-2" style={{ animationDelay: `${idx * 50}ms` }}>
                ...
                <div className="flex-1 bg-white border border-slate-200 rounded-3xl p-6 shadow-sm hover:border-blue-200 transition-all grid grid-cols-1 md:grid-cols-4 gap-6">
                  <div className="md:col-span-2">
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Unidade Técnica</label>
                    <select 
                      className="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-2.5 text-[10px] font-black uppercase text-[#0d457a]"
                      value={mov.toSector}
                  ...
                  <div>
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Dias na Unidade</label>
                    <div className="relative">
                      <input 
                        type="number"
                        min="0"
                        className="w-full bg-slate-50 border border-slate-100 rounded-xl pl-4 pr-10 py-2 text-sm font-black text-[#0d457a]"
                        value={mov.daysSpent}
                        onChange={(e) => handleUpdateMovement(idx, 'daysSpent', e.target.value)}
                      />
            ...
            <button 
              ...
              className="w-full py-4 border-2 border-dashed border-slate-200 rounded-[32px] text-slate-400 font-black uppercase text-[10px] tracking-widest hover:border-[#0d457a] hover:text-[#0d457a] transition-all flex items-center justify-center gap-2"
            >
              <Plus size={16} /> Adicionar Etapa Intermediária
            </button>
          </div>
          ...
          <div className="pt-10 border-t border-slate-100">
            <div className={`p-8 rounded-[40px] border-2 transition-all ${isFinalizing ? 'bg-blue-50 border-blue-500' : 'bg-slate-50 border-slate-200'}`}>
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-4">
                  <div className={`p-4 rounded-2xl ${isFinalizing ? 'bg-blue-500 text-white' : 'bg-slate-200 text-slate-400'}`}>
                    {isFinalizing ? <Lock size={24} /> : <CheckCircle2 size={24} />}
                  </div>
                  <div>
                    <h4 className="text-base font-black text-[#0d457a] uppercase tracking-tight">Finalização / Bloqueio</h4>
                    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1">Configurar estado de liquidação financeira</p>
                  </div>
                </div>
              ...
              {isFinalizing && (
                <div className="space-y-4 animate-in slide-in-from-top-2">
                  <label className="text-[9px] font-black text-blue-600 uppercase tracking-widest ml-1">Status de Baixa (Gera Lock no Sistema)</label>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    {finalStatuses.map(s => (
                      ...
                      >
                        ...
                        {s.name === 'EMPENHO / LIQUIDAÇÃO' && <Lock size={12} className="float-right text-blue-400" />}
                      </button>
                    ))}
                  </div>
                </div>
        ...
        <div className="p-10 border-t border-slate-100 bg-slate-50/50 flex justify-end gap-4 shrink-0">
          ...
          <button 
            onClick={handleSave}
            className="bg-[#0d457a] text-white px-12 py-4 rounded-2xl font-black uppercase text-xs tracking-widest shadow-xl hover:bg-[#0a365f] transition-all flex items-center gap-3"
          >
            <Save size={18} /> Salvar Histórico e Status
          </button>
        </div>
      </div>
    </div>
