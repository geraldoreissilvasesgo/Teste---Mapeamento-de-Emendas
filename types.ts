...
export enum Role {
  SUPER_ADMIN = 'Super Administrador', // Gestor de todos os Tenants
...
export interface Tenant {
  ...
  code: string;
  active: boolean;
  quota: number; // Limite de processos
}
...
export interface Amendment {
  id: string;
  tenantId: string; // Chave para SaaS
  code: string;
  seiNumber: string;
  year: number;
  type: AmendmentType;
  ...
  gnd?: GNDType;
  createdAt: string;
  aiInsights?: AIAnalysisResult; // Cache da análise IA
  entryDate?: string;
  healthUnit?: string;
